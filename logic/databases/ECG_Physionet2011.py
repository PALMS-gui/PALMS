...
class ECG_Physionet2011(Database):  # NB: !!!!!!!!!!!  class name should be equal to database name (this filename)
    def __init__(self):
        ...
        self.DATAPATH = resource_path(pathlib.Path(r'docs\examples\example_Physionet2011'))  # NB: top-level folder with the data
    ...
    def set_annotation_data(self):
        ...
        if existing_annotation_files is not None:
            ...
            try:
                qrs_detector = PanTompkinsQRSDetector(ecg, fs, verbose=True, log_data=False, plot_data=False, show_plot=False)
