...
class TableSplitter(QtWidgets.QSplitter):
    ...
    def eventFilter(self, source: QtCore.QObject, event: QtCore.QEvent) -> QtWidgets.QWidget.eventFilter:
...
class Frame(QtWidgets.QFrame):
    ...
    def __init__(self, main_window, *args, **kwargs):
        ...
        self.n = PALMS.config['panel_height']
        self.updateHeight()
    ...
    def updateHeight(self):
        ...
        from gui import PALMS
        PALMS.config['panel_height'] = self.n
    ...
    def mouseMoveEvent(self, event: QtGui.QMouseEvent):
        if event.buttons() != QtCore.Qt.LeftButton:
            event.ignore()
            return
        if (sub(event.pos(), self.dragStartPos)).manhattanLength() < QtWidgets.QApplication.startDragDistance():
            event.ignore()
            return
...
class DisplayPanel(QtWidgets.QWidget):
    ...
    def __init__(self, frame: Frame):
        ...
        self.table_splitter.addWidget(self.plot_area)
        ...
        # Plot area
        self.hideView.connect(self.plot_area.hideView)
        self.showView.connect(self.plot_area.showView)
        self.selectionChanged.connect(self.plot_area.selectionChanged)
        self.changeColor.connect(self.plot_area.changeColor)
        self.plotViewObj.connect(self.plot_area.addView)
    ...
    def removeViewFromChildren(self, view_to_remove):
        ...
        self.plot_area.removeView(view_to_remove)
    ...
    def addDerivative(self, parent_view, panel_index, order: int = 1):
        self.selectView(parent_view)
        self.main_window.parent()
        if order == 1:
            self.main_window.application.add_view_from_track(parent_view.track.derive_1der(), panel_index)
        elif order == 2:
            self.main_window.application.add_view_from_track(parent_view.track.derive_2der(), panel_index)
        else:
    ...
    def finishViewOperation(self, view, panel_index):
        ...
        self.main_window.model.panels[panel_index].views[-1].renderer.vb.setRange(xRange=view_range[0], yRange=view_range[1], padding=0)
        self.main_window.evalTrackMenu()
