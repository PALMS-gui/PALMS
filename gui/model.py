...
class View(object):
    track2renderers = {t.__name__: {r.name: r for r in rendering.get_renderer_classes(t)} for t in tracking.get_track_classes()}

    def __init__(self, track: tracking.Track, attached_panel: 'Panel', renderer_name: Optional[str] = None, show: bool = True,
                 color: Tuple[int, int, int] = (255, 255, 255), **parameters):
        self.track = track
        self.show = show
        self.panel = attached_panel
        self.renderer: Optional[rendering.Renderer] = None
        self.y_bounds = (0, 1)
        self.change_renderer(renderer_name if renderer_name is not None else next(iter(self.track2renderers[type(self.track).__name__])),
                             **parameters)
        self.color = color
    ...
    def change_renderer(self, renderer_name: str, **parameters):
        try:
            self.renderer = self.track2renderers[type(self.track).__name__][renderer_name](**parameters)
        except KeyError:
...
class Panel(object):
    ...
    def get_view_from_track_label(self, track_label):
        for v in self.views:  # check in current panel
            if v.track.label == track_label:
                return v
        for p in self.model.panels:  # check other panels
            for v in p.views:
                if v.track.label == track_label:
                    return v
        return None

    def new_view(self, track: tracking.Track, renderer_name: Optional[str] = None, show: bool = True,
                 color: Tuple[int, int, int] = (255, 255, 255), pos: Optional[int] = None, **parameters) -> View:
        if not pos:
            pos = len(self.views)
...
class Model(object):
    ...
    def move_view_across_panel(self, view: View, to_panel: Panel):
    ...
    @staticmethod
    def link_track_across_panel(view: View, to_panel: Panel):
        renderer_name = view.renderer.name
        new_view = to_panel.new_view(view.track, renderer_name, show=view.show, color=view.color)
        return new_view
    ...
    def copy_view_across_panel(view: View, to_panel: Panel):
        ...
        show = view.show
        new_view = to_panel.new_view(track, renderer.name, show=show, color=color)
